...
public class Test {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// ------check--------
		result = new boolean[threadnum][routenum * coachnum * seatnum + 1][stationnum];
		for (int i = 0; i < threadnum; i++) {
			for (int j = 0; j <= routenum * coachnum * seatnum; j++) {
					...
					for (int i = 0; i < testnum; i++) {
						...
						if (0 <= sel && sel < retpc && soldTicket.size() > 0) { // return ticket
							...
							if ((ticket = soldTicket.remove(select)) != null) {
								...
								if (tds.refundTicket(ticket)) {
									...
									for (int k = ticket.departure; k < ticket.arrival; k++) {
										result[ThreadId.get()][seatnum * coachnum * (ticket.route - 1)
												+ seatnum * (ticket.coach - 1) + ticket.seat][k] = false;
									}
									// ------check--------
								} else {
							...
							if ((ticket = tds.buyTicket(passenger, route, departure, arrival)) != null) {
								...
								for (int k = ticket.departure; k < ticket.arrival; k++) {
									result[ThreadId.get()][seatnum * coachnum * (ticket.route - 1)
											+ seatnum * (ticket.coach - 1) + ticket.seat][k] = true;
								}
								// ------check--------
							} else {
								System.out.println(preTime + " " + String.valueOf(System.nanoTime() - startTime) + " "
		...
		for (int j = 0; j < routenum * coachnum * seatnum; j++) {
			for (int k = 0; k < stationnum; k++) {
				for (int i = 0; i < threadnum; i++) {
					boolean tmp = false;
					if (tmp && result[i][j][k]) {
						System.out.println("[ERROR] error found!");
					}
					tmp = tmp || result[i][j][k];
				}
			}
		}
	}
