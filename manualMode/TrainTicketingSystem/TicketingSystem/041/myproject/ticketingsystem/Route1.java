...
public class Route1 {
    ...
    public Route1(int route, int coachNum, int seatNum, int stationNum) {
    ...
    public void modifySeatInfoIfValid(int departure, int arrival, int seatIndex) {
        int temp = stationNum * (stationNum - 1) / 2;
        boolean[] oldValidStatus = new boolean[temp];
        boolean[] newValidStatus = new boolean[temp];
        for (int j = 0; j < seats[seatIndex].isValidStaionToStaion.length; ++j) {
            oldValidStatus[j] = seats[seatIndex].isValidStaionToStaion[j];
        }
        if (seats[seatIndex].occupy(departure, arrival)) {
            updateTicket(oldValidStatus, newValidStatus, seats[seatIndex], -1);
        }
    ...
    public Ticket buyTicket(String passenger, int route, int departure, int arrival) {
        ...
        synchronized (this) {
            ...
            if (possibleSeatIndex != -1) {
                if (seats[possibleSeatIndex].isValid(departure, arrival)) {
                    isHaveTicket = true;
                    modifySeatInfoIfValid(departure, arrival, possibleSeatIndex);
                } else {
                    for (i = possibleSeatIndex + 1; i < seats.length; ++i) {
                        if (seats[i].isValid(departure, arrival)) {
                            isHaveTicket = true;
                            // System.out.println("车次："+ route + "座位号："+ i + "出发站：" +departure + "到达站" + arrival);
                            modifySeatInfoIfValid(departure, arrival, i);
                            break;
                        }
                    }
                    //看前面是否有人退票
                    if (!isHaveTicket) {
                        for (i = 1; i < possibleSeatIndex; ++i) {
                            if (seats[i].isValid(departure, arrival)) {
                                isHaveTicket = true;
                                modifySeatInfoIfValid(departure, arrival, i);
                                break;
