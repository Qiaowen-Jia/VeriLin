...
public class Test {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for (int t = 0; t < threadNums.length; t++) {
			final TicketingDS tds = new TicketingDS(ROUTE_NUM, COACH_NUM, SEAT_NUM, STATION_NUM, threadNums[t]);
			...
			long buyTotalTime = calculateTotal(buyTicketTime, threadNums[t]);
			long refundTotalTime = calculateTotal(refundTime, threadNums[t]);
			long inquiryTotalTime = calculateTotal(inquiryTime, threadNums[t]);

			double bTotal = (double) calculateTotal(buyTotal, threadNums[t]);
			double rTotal = (double) calculateTotal(refundTotal, threadNums[t]);
			double iTotal = (double) calculateTotal(inquiryTotal, threadNums[t]);
			...
			long tho = (long) (threadNums[t] * TEST_NUM / (double) time) * 1000; // 1000是从ms转换为s
			System.out.println(String.format(
					"ThreadNum: %d BuyAvgTime(ns): %d RefundAvgTime(ns): %d InquiryAvgTime(ns): %d ThroughOut(t/s): %d",
					threadNums[t], buyAvgTime, refundAvgTime, inquiryAvgTime, tho));
			clear();
		}
	}
