...
public class TicketingDSAd implements TicketingSystem
{
	//车站信息
	int routenum = 5;//车次数
	int coachnum = 8;//车厢数
	...
	void OtherInit()
	{
		...
		groupNum = Math.max((int)(Math.sqrt(allSeat)/2), (threadnum-1)/routenum+1);
		if(groupNum<8)
			groupNum = 1;
	...
	//构造函数
	public TicketingDSAd()
	{
		OtherInit();
	}
	public TicketingDSAd(int routenum, int coachnum, int seatnum, int stationnum, int threadnum)
	{
