...
public class Test {
	...
	final static int[] threadnums = {2, 4, 8, 16, 32, 64}; // concurrent thread number
	...
	public static void work(int threadnum)throws InterruptedException {
		...
		System.out.println("AvgRet_time: "+ (totalRetTime.get() / totalRetNum.get()) + " ns/op" +
				"\tAvgBuy_time: " + (totalBuyTime.get() / totalBuyNum.get()) + " ns/op" +
				"\tAvgQuery_time: " + (totalQueryTime.get() / totalQueryNum.get()) + " ns/op");
		System.out.println("TPS: " + (double)(testnum * threadnum) / executionTime + " kop/s");
		System.out.println("\n\n");
	}
}
