...
public class ConfigParser{
  ...
  public static boolean ReversibleRelationFlag;
  ...
  public static List<MethodDeclaration> testSearchMethods;
  public static List<MethodDeclaration> testModifyMethods;


  public ConfigParser(){
	ObjectFlag = false;
	FileTypeFlag = false;
	ReversibleRelationFlag = false;
	...
	testSearchMethods = new ArrayList<MethodDeclaration>();
	testModifyMethods = new ArrayList<MethodDeclaration>();
  }
  ...
  public static boolean isValid(){
	if(ObjectFlag && FileTypeFlag && MethodListFlag && MethodFreqFlag && ReversibleRelationFlag){
...
//		rMethods.addMethod(m);
  ...
  public static boolean parseSearchMethods(String str){
	if(!MethodListFlag){
	  System.out.println("There is no MethodList declared before SearchMethods.");
	  return false;
	}
	String[] splitstr = str.split(":");
	if(splitstr.length != 2){
	  System.out.println("Syntax Error in setting SearchMethods of configuration.");
	  return false;
	}
	if(!elimAllSymbol(splitstr[0]).equals("SearchMethods")){
	  System.out.println("Syntax Error in SearchMethods.");
	  return false;
	}
	String list = splitstr[1].trim();
	String[] splitlist = list.split(",");
	for(int i = 0; i < splitlist.length; i++){
	  ...
	  if(md == null){
		...
		rMethods.testSearchMethodList.add(m);
	  }	
	}
	return true;
  }

  public static boolean parseModifyMethods(String str){
	if(!MethodListFlag){
	  System.out.println("There is no MethodList declared before ModifyMethods.");
	  return false;
	}
	String[] splitstr = str.split(":");
	if(splitstr.length != 2){
	  System.out.println("Syntax Error in setting ModifyMethods of configuration.");
	  return false;
	}
	if(!elimAllSymbol(splitstr[0]).equals("ModifyMethods")){
	  System.out.println("Syntax Error in ModifyMethods.");
	  return false;
	}
	String list = splitstr[1].trim();
	String[] splitlist = list.split(",");
	for(int i = 0; i < splitlist.length; i++){
	  ...
	  if(md == null){
		...
		Method m = rMethods.getMethodByName(method);
		rMethods.testModifyMethodList.add(m);
	  }
	}
	return true;
  }
...
//	  System.out.println(f.getPath());
  ...
  // here the reversible might be itself
  public static boolean parseReversibleRelation(String str){
	if(rMethods.reversibleMethodList == null){
	  if(rMethods.hasSpec && rMethods.specMethodList != null){
		rMethods.reversibleMethodList = rMethods.specMethodList;
	  }
	  else{
		if(!rMethods.hasSpec){
		  rMethods.reversibleMethodList = methods;
		}
		else{
		  System.out.println("The spec should be defined before reversible in the configuration.");
		  return false;
		}
	  }
	}
	String[] splitstr = str.split(":");
	if(splitstr.length != 2){
	  System.out.println("Syntax Error in setting reversible relation of configuration.1");
	  return false;
	}
	if(!elimAllSymbol(splitstr[0]).equals("ReversibleRelation")){
	  System.out.println("Syntax Error in ReversibleRelation.");
	  return false;
	}
	String[] splitline = splitstr[1].trim().split(",");
	if(splitline.length != 2){
	  System.out.println("Syntax Error in setting reversible relation of configuration.2");
	  return false;
	}
	String mainname = elimAllSymbol(splitline[0]);
	String reversiblename = elimAllSymbol(splitline[1]);
	Method m = null;
...
/*	  if(!SpecFileFlag){
	  ...
	  m.setReversible(reversiblename);
	}

	for(int i = 0; i < rMethods.reversibleMethodList.size(); i++){
	  if(rMethods.reversibleMethodList.get(i).getNameAsString().equals(reversiblename)){
		m.setReversibleMethod(rMethods.reversibleMethodList.get(i));
		break;
	  }
	  if(i == rMethods.reversibleMethodList.size() - 1){
		System.out.println("There is no method named " + reversiblename + " in File " + rMethods.reversibleObject);
		return false;
	  }
	}
	boolean flag = true;
	for(int i = 0; i < rMethods.methodNum; i++){
	  if(rMethods.testMethodList.get(i).reversible == null){
//		System.out.println(rMethods.testMethodList.get(i).methodName + " dual false");
		flag = false;
	  }
	}
	ReversibleRelationFlag = flag;
	return true;
  }
  public static boolean parseSpecRelation(String str){
	...
	String[] splitstr = str.trim().split(":");
	if(splitstr.length != 2){
	  System.out.println("Syntax Error in setting reversible relation of configuration.");
	  return false;
	}
...
/*		else if(line.contains("DualFileName")){
		...
		else if(line.contains("ReversibleRelation")){
		  flag = parseReversibleRelation(line);
		  if(flag){
			System.out.println("Set ReversibleRelation successfully.");
		  }
		  else{
			System.out.println("Syntax Error in line: " + count + ". Set ReversibleRelation incorrect.");
			break;
		  }
		}
		...
		else if(line.contains("SearchMethods")){
		  flag = parseSearchMethods(line);
		  if(flag){
			System.out.println("Set SearchMethods successfully.");
		  }
		  else{
			System.out.println("Syntax Error in line: " + count + ". Set SearchMethods incorrect.");
			break;
		  }
		}
		else if(line.contains("ModifyMethods")){
		  flag = parseModifyMethods(line);
		  if(flag){
			System.out.println("Set ModifyMethods successfully.");
		  }
		  else{
			System.out.println("Syntax Error in line: " + count + ". Set ModifyMethosd incorrect.");
			break;
		  }
		}
		...
		else if(line.contains("ReadMethod")){
		  flag = parseSearchMethods(line);
		  if(flag){
			System.out.println("Set ReadMethod successfully.");
		  }
		  else{
			System.out.println("Syntax Error in line: " + count + ". Set ReadMethod incorrect.");
			break;
		  }
		}
		else if(line.contains("WriteMethod")){
		  flag = parseModifyMethods(line);
		  if(flag){
			System.out.println("Set writeMethod successfully.");
		  }
		  else{
			System.out.println("Syntax Error in line: " + count + ". Set writeMethod incorrect.");
			break;
		  }
