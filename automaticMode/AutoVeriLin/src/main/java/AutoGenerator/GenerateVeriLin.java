...
//	System.out.println("testMethodNum = " +  rMethods.testMethodList.size());
  ...
  public static boolean generateCheckLine_spec(RMethods rMethods){
	...
	for(int i = 0; i < rMethods.testMethodList.size(); i++){
	  ...
	  MethodDeclaration reversible = m.reversible;
	  System.out.println("if(tl.actionName.equals(\"" + m.methodName +"\")){");
	  if(m.parameters.isEmpty()){
		if(m.type.isVoidType()){//no parameter && no type
...
//			System.out.println(" m.methodname empty" + m.methodName);
		  ...
		  if(m.type.equals(spec.getType())){
			...
			print1 = "			object." + m.reversible.getNameAsString() + "(";
		  }
		  else{
			System.out.println("Error: The type in " + m.methodName + " is not same to that in spec method " + spec.getNameAsString());
			return false;
		  ...
		  if(isPrimitive(m.type.toString()))
			if(m.type.toString().equals("boolean")){
			  System.out.println("	if((res && tl.p_" + i + "_res.equals(\"False\")) ||(!res && tl.p_" + i + "_res.equals(\"True\"))){");
			  System.out.println("		if(tl.p_" + i + "_res.equals(\"False\")){");
			  if(m.reversible.getParameters().isEmpty()){
				System.out.println("			object." + m.reversible.getNameAsString() + "();");
			  }
			  else{
  ...
  public static boolean generateBackLine(RMethods rMethods){
	...
	for(int i = 0; i < rMethods.testMethodList.size(); i++){
	  Method m = rMethods.testMethodList.get(i);
	  MethodDeclaration reversible = m.reversible;//the parameter name is correspond to that of 
	  System.out.println("	if(tl.actionName.equals(\"" + m.methodName +"\")){");
	  if(reversible.getParameters().isEmpty()){
		if(reversible.getType().isVoidType()){//no parameter && no type
		  System.out.println("		object." + reversible.getNameAsString() + "();");
		}
		else{// no paramter && has type
		  System.out.println("		" + reversible.getTypeAsString() + " res = object." + reversible.getNameAsString() + "();");
		  if(reversible.getTypeAsString().equals("boolean")){
		...
		if(reversible.getType().isVoidType()){//has parameter && no type
		  String print = "		object." + reversible.getNameAsString() + "(";
		  for(int j = 0; j < reversible.getParameters().size(); j++){
			String pname = reversible.getParameter(j).getNameAsString();
			Type ptype = reversible.getParameter(j).getType();
			if(m.parameters.size() == 0 && reversible.getParameter(j).getType().equals(m.type)){
			  ...
			  for(int k = 0; k < m.parameters.size(); k++){
				...
				if(k == m.parameters.size() - 1){
				  System.out.println("There is no matching parameter named " + pname + " in method " + reversible.getNameAsString());
				  return false;
				}
			  }
			}
			if(j != reversible.getParameters().size() - 1)
			  print = print + ", ";
		  ...
		  System.out.println("		return true;");
		  System.out.println("	}");
		  String print = "	" + reversible.getTypeAsString() + " res = object." + reversible.getNameAsString() + "(";
		  for(int j = 0; j < reversible.getParameters().size(); j++){
			String pname = reversible.getParameter(j).getNameAsString();
			Type ptype = reversible.getParameter(j).getType();
			if(pname.equals("result") && reversible.getParameter(j).getType().equals(m.type))
			  print = print + "tl.p_" + i + "_res";
			else{
			  for(int k = 0; k < m.parameters.size(); k++){
				if(pname.equals(m.parameters.get(k).getNameAsString()) && ptype.equals(m.parameters.get(k).getType())){
...
/*  public static boolean generateBackLine_seq(RMethods rMethods){
  ...
  public static boolean generateFilterModify(RMethods rMethods){
	System.out.println("public static boolean filter_modify(ArrayList<TraceLine> trace, ArrayList<TraceLine> trace_modify, ArrayList<TraceLine> trace_search){");
	System.out.println("	for(int i = 0; i < trace.size(); i++){");
	System.out.println("		TraceLine tl = trace.get(i);");
	for(int i = 0; i < rMethods.testModifyMethodList.size(); i++){
	  System.out.println("		if(tl.action.equals(\"" + rMethods.testModifyMethodList.get(i).methodName + "\"){");
	  System.out.println("			trace_modify.add(tl);");
	  System.out.println("		else");
	  System.out.println("			trace_search.add(tl);");
	  System.out.println("		}");
	}
	return true;
  }
  public static boolean generateNewCheckSearchLine(RMethods rMethods){
	System.out.println("public static int new_check_search_line(" + rMethods.specObject + " tds, int refion_index, ArrayList<TraceLine> trace_search, int[] check, int index, int old_line){");
	System.out.println("	TraceLine tl = trace_search.get(index);");
	System.out.println("	if(check[index] != -1)");
	System.out.println("		if(index == old_line)");
	System.out.println("			return old_line + 1;");
	System.out.println("		else");
	System.out.println("			return old_line;");
	for(int i = 0; i < rMethods.testSearchMethodList.size(); i++){
	  Method m = rMethods.testSearchMethodList.get(i);
	  System.out.println("	if(tl.actionName.equals(\"" + m.methodName + "\")){");
	  int count=0;
	  for(int j = 0; j < rMethods.testMethodList.size(); j++){
		if(rMethods.testMethodList.get(j).methodName.equals(rMethods.testSearchMethodList.get(i).methodName)){
		  count = j;
		}
	  }
	  System.out.println("		if( r == p_" + count + "_res){");
	  System.out.println("			check[index] = region_index;");
	  System.out.println("			if(index == old_line)");
	  System.out.println("				return old_line + 1;");
	  System.out.println("			else{");
	  System.out.println("				return old_line;");
  ...
  public static boolean generateMain(RMethods rMethods){
	return true;
  }
  ...
  public static void main(String[] args){
	...
	try{
	  ...
	  if(!generateTraceStructure(rMethods)){
		System.out.println("Error: Generating TraceStructure.");
		return;
	  }
	  ...
	  if(!generateObject(rMethods)){
		System.out.println("Error: Generating Object.");
		return;
	  }
	  if(!generateParseLine(rMethods)){
		System.out.println("Error: Generating ParseLine.");
		return;
	  }
	  if(!generateBackLine(rMethods)){
		...
		if(!generateCheckLine_spec(rMethods)){
		  System.out.println("Error: Generating CheckLine.");
		  return;
		}
	  }
	  else{
...
/*	  if(!generateFilterModify(rMethods)){
		System.out.println("Error: Generating FilterModify.");
		return;
	  }*/
	  if(!generateBackLine(rMethods)){
		System.out.println("Error: Generating BackLine.");
		return;
	  }
	  ...
	  if(!generateWriteLine(rMethods)){
		System.out.println("Error: Generating WriteLine.");
		return;
	  }
...
/*	  if(!generateNewCheckSearchLine(rMethods)){
		System.out.println("Error: Generating New_Check_Search_Line.");
		return;
	  }*/
	  if(!copyFile("src/main/java/LinPart/region.part4")){
		System.out.println("Error: copying region.part4.");
		return;	  
	  }
	  if(!generateMain(rMethods)){
		System.out.println("Error: Generating Main.");
		return;
	  }
/*	  if(!copyFile("src/main/java/LinPart/region.part5")){
		System.out.println("Error: copying region.part5.");
		return;	  
	  }*/
