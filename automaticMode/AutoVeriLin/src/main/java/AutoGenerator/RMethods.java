...
public class RMethods{
  ...
  public List<Method> testSearchMethodList;
  public List<Method> testModifyMethodList;
  ...
  public String reversibleObject;
  public List<MethodDeclaration> reversibleMethodList;
  ...
  public RMethods(){
	...
	testModifyMethodList = new ArrayList<Method>();
	testSearchMethodList = new ArrayList<Method>();
  ...
  public void setReversibleFileName(String str){
	String[] split = str.split(".");
	if(split.length !=2){
	  System.out.println("Error in resolving SpecFileName.");
	  return;
	}
	reversibleObject = str;
	specObject = split[0].trim();
  }
  ...
  public void addSearchMethod(String name){
	  Method m = getMethodByName(name);
	  if(m == null){
		System.out.println("Error: There is no method named " + name + " in " + objectName);
		return;
	  }
	  testSearchMethodList.add(m);
  }
  public void addModifyMethod(String name){
	  Method m = getMethodByName(name);
	  if(m == null){
		System.out.println("Error: There is no method named " + name + " in " + objectName);
		return; 
	  }
	  testModifyMethodList.add(m);
  }
  ...
  public void printstate(){
	...
	for(int i = 0; i < methodNum; i++){
	  ...
	  System.out.println("reversibleMethod: "+ m.reversibleName + ", specMethod: " + m.specName);
